...
exports.fetchFeaturedTempImages = () => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM `product_template` INNER JOIN `product_temp_images` ON `product_template`.`temp_id` = `product_temp_images`.`product_temp_id` WHERE `product_temp_images`.`featured_image` = 1",
      (err, res) => {
        if (err) {
          reject(err);
        } else {
...
exports.fetchCurrencyRate = (currencyCode) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(
      "SELECT `rate` FROM `exchange_rates` WHERE `sign` = ?",
      [currencyCode],
      (err, res) => {
        if (err) {
          reject(err);
...
exports.fetchTaxRate = (currencyCode) => {
  return new Promise((resolve, reject) => {
    // console.log("curr:", currencyCode)
    db.query(
      "SELECT `tax_percentage` FROM `exchange_rates` WHERE `sign` = ?",
      [currencyCode],
      (err, res) => {
        if (err) {
          reject(err);
...
exports.fetchAllUserInfo = () => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT `user`.`user_id`, `user`.`user_name`, `user`.`user_email`, `user`.`reference_id`, `user`.`phone`, `shop`.`id` FROM `user` INNER JOIN `shop` ON `shop`.`seller_user_id` = `user`.`user_id`",
      (err, res) => {
        if (err) {
          reject(err);
        } else {
...
exports.fetchAllNotifications = (userID) => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM `notification` WHERE `user_id` = ? ORDER BY `notification`.`notification_id` DESC",
      [userID],
      (err, res) => {
        if (err) {
          reject(err);
...
exports.setNotification = (userID, notification, url) => {
  return new Promise((resolve, reject) => {
    db.query(
      "INSERT INTO `notification` (`notification_id`, `user_id`, `notification`, `url`, `is_read`) VALUES (NULL, ?, ?, ?, '0')",
      [userID, notification, url],
      (err, res) => {
        if (err) {
          reject(err);
        } else {
