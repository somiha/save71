...
exports.getPopularManufacturer = async (req, res) => {
  ...
  if (isLogged) {
    try {
      ...
      const products = await new Promise((resolve, reject) => {
        db.query(
          "SELECT * FROM `products` INNER JOIN `extra_cat` ON `extra_cat`.`extra_cat_id` = `products`.`product_cat_id` WHERE `products`.`quantity` = 0 AND `products`.`status` = 1 AND `products`.`admin_published` = 1 ORDER BY `products`.`sell_count` DESC",
          (err, productsData) => {
            if (err) {
              reject(err);
            } else {
    ...
    try {
      ...
      if (isLogged) {
        ...
        const location = JSON.parse(
          JSON.stringify(req.cookies.location) || "{}"
        );
        const userLat = crypto.decrypt(location.latitude);
        const userLng = crypto.decrypt(location.longitude);
