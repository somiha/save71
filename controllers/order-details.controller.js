...
exports.order_status = async (req, res) => {
  try {
    ...
    const seller = await queryAsync("SELECT * FROM `shop` WHERE `id` = ?", [
      orders[0].seller_id,
    ]);
    if (seller.length === 0) {
    ...
    const sellerUser = await queryAsync(
      "SELECT * FROM `user` WHERE `user_id` = ?",
      [seller[0].seller_user_id]
    );
    if (sellerUser.length === 0) {
      return res.status(404).send("Seller user details not found.");
    }

    // Fetch reference user details
    const refUser = await queryAsync(
      "SELECT * FROM `user` WHERE `own_ref_id` = ?",
      [sellerUser[0].reference_id]
    );
    if (refUser.length === 0) {
